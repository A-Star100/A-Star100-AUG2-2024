...
public class PagerSnapHelper extends SnapHelper {
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX, int velocityY) {
        ...
        View mStartMostChildView = findStartView(layoutManager2, orientationHelper);
        if (mStartMostChildView == null || (centerPosition = layoutManager2.getPosition(mStartMostChildView)) == -1) {
            return -1;
        }
        boolean z = false;
        if (layoutManager.canScrollHorizontally()) {
        ...
        if (centerPosition == 0 && forwardDirection == reverseLayout && distanceToCenter(layoutManager2, mStartMostChildView, orientationHelper) == 0) {
            return -1;
        }
        if (reverseLayout) {
            if (forwardDirection) {
