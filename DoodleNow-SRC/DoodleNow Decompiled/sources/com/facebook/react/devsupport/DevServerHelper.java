...
public class DevServerHelper {
    ...
    private String getInspectorDeviceId() {
        ...
        return getSHA256(String.format(Locale.US, "android-%s-%s", new Object[]{str, "android_id"}));
    }

    /* access modifiers changed from: private */
    public String getInspectorDeviceUrl() {
        return String.format(Locale.US, "http://%s/inspector/device?name=%s&app=%s&device=%s", new Object[]{this.mPackagerConnectionSettings.getDebugServerHost(), Uri.encode(AndroidInfoHelpers.getFriendlyDeviceName()), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId())});
    }
    ...
    public void openDebugger(final ReactContext reactContext, final String str) {
        this.mClient.newCall(new Request.Builder().url(String.format(Locale.US, "http://%s/open-debugger?appId=%s&device=%s", new Object[]{this.mPackagerConnectionSettings.getDebugServerHost(), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId())})).method(ShareTarget.METHOD_POST, RequestBody.create((MediaType) null, "")).build()).enqueue(new Callback() {
            public void onResponse(Call call, Response response) {
            }
