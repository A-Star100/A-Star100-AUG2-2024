...
public class UnicodeExtensionKeys {
    ...
    public static boolean isValidKeyword(String str, String str2, ILocaleObject iLocaleObject) throws JSRangeErrorException {
        if (Build.VERSION.SDK_INT >= 24) {
            ULocale m = Intl$$ExternalSyntheticApiModelOutline0.m(iLocaleObject.getLocale());
            String[] strArr = new String[0];
            if (str.equals(Constants.COLLATION_EXTENSION_KEY_SHORT)) {
                if (str2.equals(Constants.COLLATION_STANDARD) || str2.equals("search")) {
                    return false;
                }
                strArr = RuleBasedCollator.getKeywordValuesForLocale(Constants.COLLATION_EXTENSION_KEY_SHORT, m, false);
            } else if (str.equals("ca")) {
                strArr = Calendar.getKeywordValuesForLocale("ca", m, false);
            } else if (str.equals("nu")) {
                strArr = ReactTextView$$ExternalSyntheticApiModelOutline0.m();
            }
            if (strArr.length == 0) {
                return true;
            }
