...
abstract class SessionProcessorBase implements SessionProcessor, CameraExtensionsInfo, CameraExtensionsControl {
    ...
    public /* synthetic */ Map getSupportedPostviewSize(Size size) {
        return SessionProcessor.CC.$default$getSupportedPostviewSize(this, size);
    }
    ...
    private Set<Integer> getSupportedCameraOperations(List<CaptureRequest.Key> list) {
        ...
        if (Build.VERSION.SDK_INT >= 30) {
            if (list.contains(ComponentDialog$$ExternalSyntheticApiModelOutline0.m()) || list.contains(CaptureRequest.SCALER_CROP_REGION)) {
                hashSet.add(0);
            }
        ...
        if (Build.VERSION.SDK_INT >= 34 && list.contains(ComponentDialog$$ExternalSyntheticApiModelOutline0.m$2())) {
            hashSet.add(8);
        }
        return hashSet;
    }
