...
public class BasicVendorExtender implements VendorExtender {
    ...
    static {
        ...
        if (Build.VERSION.SDK_INT >= 30) {
            arrayList.add(ComponentDialog$$ExternalSyntheticApiModelOutline0.m());
        }
    }
    ...
    public List<Pair<Integer, Size[]>> getSupportedCaptureOutputResolutions() {
        ...
        return Arrays.asList(new Pair[]{new Pair(256, getOutputSizes(getCaptureInputImageFormat()))});
    }
    ...
    private List<Pair<Integer, Size[]>> replaceImageFormatIfMissing(List<Pair<Integer, Size[]>> list, int i, int i2) {
        ...
        for (Pair next : list) {
            if (((Integer) next.first).intValue() == i) {
                arrayList.add(new Pair(Integer.valueOf(i2), (Size[]) next.second));
                z = true;
            } else {
                arrayList.add(next);
            }
