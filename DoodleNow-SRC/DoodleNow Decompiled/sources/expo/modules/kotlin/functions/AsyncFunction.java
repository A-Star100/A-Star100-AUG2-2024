...
public abstract class AsyncFunction extends BaseAsyncFunctionComponent {
    ...
    private final void dispatchOnQueue(AppContext appContext, Function0<Unit> function0) {
        ...
        if (i == 1) {
            for (AnyType kType : getDesiredArgsTypes()) {
                KClassifier classifier = kType.getKType().getClassifier();
                KClass kClass = classifier instanceof KClass ? (KClass) classifier : null;
                if (kClass != null) {
                    if (View.class.isAssignableFrom(JvmClassMappingKt.getJavaClass(kClass))) {
