...
public final class ColorTypeConverter extends DynamicAwareTypeConverters<Color> {
    ...
    private final Color colorFromDoubleArray(double[] dArr) {
        ...
        Color m = Color.valueOf((float) dArr[0], (float) dArr[1], (float) dArr[2], (float) (orNull != null ? orNull.doubleValue() : 1.0d));
        Intrinsics.checkNotNullExpressionValue(m, "valueOf(...)");
        return m;
    }

    private final Color colorFromInt(int i) {
        Color m = Color.valueOf(i);
        Intrinsics.checkNotNullExpressionValue(m, "valueOf(...)");
        return m;
    }
    ...
    private final Color colorFromString(String str) {
        ...
        if (list != null) {
            Color m = Color.valueOf(((Number) list.get(0)).floatValue(), ((Number) list.get(1)).floatValue(), ((Number) list.get(2)).floatValue(), ((Number) list.get(3)).floatValue());
            Intrinsics.checkNotNullExpressionValue(m, "valueOf(...)");
            return m;
        }
        Color m2 = Color.valueOf(Color.parseColor(str));
        Intrinsics.checkNotNullExpressionValue(m2, "valueOf(...)");
        return m2;
    }
