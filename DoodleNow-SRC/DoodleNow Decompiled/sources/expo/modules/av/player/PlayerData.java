...
public abstract class PlayerData implements AudioEventHandler {
    ...
    public void lambda$setEnableSampleBufferCallback$0(boolean z) {
        if (!UiThreadUtil.isOnUiThread()) {
            UiThreadUtil.runOnUiThread(new PlayerData$$ExternalSyntheticLambda2(this, z));
        } else if (z) {
            try {
                if (!this.mAVModule.hasAudioPermission()) {
                    this.mAVModule.requestAudioPermission(new PlayerData$$ExternalSyntheticLambda3(this));
                    return;
                }
    ...
    /* access modifiers changed from: private */
    public void emitSampleBufferEvent(byte[] bArr, double d) {
        UIManager uIManager = (UIManager) this.mUiManager.get();
        if (uIManager != null) {
            uIManager.runOnClientCodeQueueThread(new PlayerData$$ExternalSyntheticLambda0(this, bArr, d));
        }
    }
