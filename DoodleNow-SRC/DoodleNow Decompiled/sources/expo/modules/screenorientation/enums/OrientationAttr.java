...
public enum OrientationAttr implements Enumerable {
    ...
    private OrientationAttr(int i) {
        this.value = i;
    }

    static {
        OrientationAttr[] $values;
    ...
    public final OrientationLock toOrientationLock$expo_screen_orientation_release() {
        try {
            for (OrientationLock orientationLock : OrientationLock.values()) {
                if (Intrinsics.areEqual((Object) orientationLock.name(), (Object) name())) {
    ...
    public static final class Companion {
        ...
        public final OrientationAttr fromInt(int i) throws InvalidArgumentException {
            try {
                for (OrientationAttr orientationAttr : OrientationAttr.values()) {
                    if (orientationAttr.getValue() == i) {
                        return orientationAttr;
                    }
                }
                throw new NoSuchElementException("Array contains no element matching the predicate.");
            } catch (NoSuchElementException unused) {
                throw new InvalidArgumentException("Platform orientation " + i + " is not a valid Android orientation attr");
            }
        }
    }
}
